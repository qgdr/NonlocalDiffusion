...
%     &= \frac{1}{\Gamma(2-\alpha)} \int_{x_{j-1}}^{x_{j+1}} |x_i-y|^{1-\alpha} \phi_j(y) dy \\
...
We have replaced \((-\Delta)^{\alpha/2} u(x_i ) = f (x_i )\) in \cref{eq:equation} by \(-\kappa_\alpha u_h(x_i ) = f (x_i )\) in \cref{def:discrete_equation}, 
...
\begin{equation} \label{eq:truncerrordepart}
  \begin{aligned}
    - \kappa_\alpha D_h^{\alpha} \Pi_h u(x_i) - f(x_i)
     ...
     & = - \kappa_\alpha (D_h^2 - \frac{d^2}{dx^2}) I^{2-\alpha} u(x_i) - \kappa_\alpha D_h^2 I^{2-\alpha} (\Pi_h u - u)(x_i)  \\
  \end{aligned}
\end{equation}
