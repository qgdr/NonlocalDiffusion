...
\begin{proof}[Proof of \cref{lmm:estimatesofy-xi1-a}] \label{prf:estimatesofy-xi1-a}
  \begin{equation}
    ...
    \end{aligned}
  \end{equation}
  where \(y_{j-i-1}(\xi)-\xi\) and \(y_{j-i}(\xi)-\xi\) have the same sign (\(\ge 0\) or \(\le 0\)).

  Since \(|{y_{j-i}}(\xi)-\xi| \simeq |x_j - x_i| \), we have
  ...
  \textcolor{purple}{
  \begin{equation*}
    |{y_{j-i}^\theta}(\xi)-\xi|^{1-\alpha} \simeq |y_j^\theta - x_i|^{1-\alpha}
  \end{equation*}
  }
  Next,
  \begin{equation}
