...
\par
Our main idea to esitmate \(R_i\) is to depart it by \(V_{ij}\) and \(S_{ij}\).

For \(3\le i < N/2\), let's denote \(k=\lceil\frac{i}{2}\rceil\), then
...
\begin{figure}[htbp]
  ...
  \caption{The departure of \(R_i\) for \(3\le i < N/2\)}
\end{figure}
...
Then we esrimate each part from easy to hard.
...
To estimate \(S_{ij}\), we first estimate \(D_h^2 P_{j-i}^\theta(x_i)\) , but
