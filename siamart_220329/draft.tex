...
\begin{lemma} \label{thm:regularity-u}
  ...
  Assume that $f\in C^\beta(\Omega)$ be such that \( \|f\|_{\beta}^{(\alpha/2)} < \infty\),
  % be such that neither \(\beta\) nor \(\beta+\alpha\) is an integer.
  and $u\in C^{\alpha/2}(\mathbb{R})$ be a solution of \eqref{eq:equation}. 
  Then 
  for \(l=0, 1, 2, 3, 4\)
  \begin{equation}
    |u^{(l)}(x)| \le C 
...
\begin{proof}
  ...
  \begin{equation*}
    \|u\|_{\beta+\alpha}^{(-\alpha/2)} \le C \left( \|u\|_{C^{\alpha/2}(\mathbb{R})} + \|f\|_{\beta}^{(\alpha/2)}  \right).
  \end{equation*}
  And by \cref{def:delta-Holder-norm}
  \begin{equation*}
    \sum_{l=1}^{4} \sup_{x\in \Omega} \left( \delta(x)^{l-\alpha/2} |w^{(l)}(x)| \right) \le C \left( \|u\|_{C^{\alpha/2}(\mathbb{R})} + \|f\|_{\beta}^{(\alpha/2)}  \right),
  \end{equation*}
...
\begin{lemma} \label{thm:regularity-f}
  Let $\beta=4-\alpha+\gamma$ with \(0<\gamma<\alpha-1\).
  Assume that $f\in C^\beta(\Omega)$ be such that \( \|f\|_{\beta}^{(\alpha/2)} < \infty\), then for \(l=0, 1, 2\)
  \begin{equation}
    |f^{(l)}(x)| \le C
    \delta(x)^{-l-\alpha/2}.
  \end{equation}
...
$f\in C^\beta(\Omega)$ be such that \( \|f\|_{\beta}^{(\alpha/2)} < \infty\), and solution \(u\in C^{\alpha/2}(\mathbb{R})\). 
