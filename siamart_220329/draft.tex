...
We shall finally introduce some notations.
...
\begin{definition} [Mesh Transport Functions]
  ...
  \begin{equation} \label{def:yij}
    ...
    \end{cases}
  \end{equation}
  where  \( Z_{j} := T^{1/r}\frac{j}{N} \), \(x\in [x_{i-1}, x_{i+1}]\).
  And
  ...
  \begin{equation} \label{def:Qj-itheta-jlN}
    {Q_{i,j}^\theta}(x) = ({h_{i,j}}(x))^4 \frac{|{y_{i,j}^\theta}(x)-x|^{1-\alpha}}{\Gamma(2-\alpha)}
  \end{equation}
\end{definition}
}
  ...
  \begin{equation} \label{lmm:Tij-express-as-int-of-function}
    \begin{aligned}
      T_{ij} & = \int_{0}^{1} -\frac{\theta (1-\theta)}{2} {P_{i,j}^\theta}(x_i) d\theta    \\
             & + \int_{0}^{1} \frac{\theta (1-\theta)}{3!}{Q_{i,j}^\theta}(x_i) \left[ \theta^2  u'''(\eta_{j,1}^\theta) - (1-\theta)^2 u'''(\eta_{j,2}^\theta) \right] d\theta
    \end{aligned}
  \end{equation}
  Immediately, we can see from \eqref{def:Sij} that
  ...
  \begin{equation} \label{lmm:Vij-int}
    \begin{aligned}
      S_{ij}
             ...
             & \quad +  \int_{0}^1 \frac{\theta^3 (1-\theta)}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i,j}^\theta}(x_{i+1}) u'''(\eta_{j+1,1}^\theta) - {Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j,1}^\theta)}{h_{i+1}}\right)  d\theta \\
             & \quad -  \int_{0}^1 \frac{\theta^3 (1-\theta)}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j,1}^\theta) - {Q_{i,j}^\theta}(x_{i-1}) u'''(\eta_{j-1,1}^\theta)}{h_{i}}\right)  d\theta   \\
             & \quad -  \int_{0}^1 \frac{\theta (1-\theta)^3}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i,j}^\theta}(x_{i+1}) u'''(\eta_{j+1,2}^\theta) - {Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j,2}^\theta)}{h_{i+1}}\right)  d\theta \\
             & \quad +  \int_{0}^1 \frac{\theta (1-\theta)^3}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j,2}^\theta) - {Q_{i,j}^\theta}(x_{i-1}) u'''(\eta_{j-1,2}^\theta)}{h_{i}}\right)  d\theta
    \end{aligned}
  \end{equation}
\end{lemma}
...
\begin{lemma}
  For \(1\le i \le 2N-1, 2\le j \le 2N-1\) and \(\xi \in (x_{i-1}, x_{i+1})\)
  \begin{equation}
    y_{i,j}(\xi)\simeq x_j, \quad h_{i,j}(\xi) \simeq h_j
  \end{equation}
  For \(1\le i, j \le 2N-1\),
  \begin{equation}
    |y_{i,j}(\xi) - \xi| \simeq |x_j - x_i|
  \end{equation}
  And for \(1\le i\le 2N-1, 2\le j \le 2N-1\),
  \begin{equation}
...
\begin{lemma} \label{lmm:derivatives-of-MTFs}
  \begin{equation}
    y_{i,j}'(x) = \begin{cases}
      ...
      (2T-y_{i,j}^{1-1/r}(x)) x^{1/r-1}   & i< N, j>N \\
  ...
  \begin{equation}
    y_{i,j}''(x) = \frac{1-r}{r}
      \begin{cases}
        y_{i,j}^{1-2/r}(x) x^{1/r-2} Z_{i-j}   & i< N, j< N \\
        \dfrac{h_N}{r Z_1} x^{1/r-2}  & i< N, j=N \\
        (2T-y_{i,j}^{1-2/r}(x)) x^{1/r-2} Z_{2N-j+i}   & i< N, j>N \\
...
% \textcolor{red}{
\begin{lemma} \label{lmm:esitmate-of-MTFs}
  For  \(2\le i \le N, 2\le j \le 2N-1\)
  \begin{equation}
    |h_{i,j}'(\xi)| \le C (r-1) h x_i^{1/r-1}
    \begin{cases}
      x_j^{1-2/r}   & j\le N \\
      (2T-x_j)^{1-2/r}    & j> N \\
    \end{cases}
  ...
  And
  \begin{equation}
    \left|(y_{i,j}(\xi) - \xi)'\right| \le C (r-1) |y_{i,j}(\xi) - \xi| \xi^{-1} \simeq (r-1) |x_j - x_i|x_i^{-1}
  \end{equation}
\end{lemma}
% }
...
Then we esrimate each part of \eqref{eq:depart-Ri} from easy to hard.
...
\textcolor{blue}{
  ...
  \begin{proof}
    For  \(\max\{2i+1, 4\} \le j \le N\), by \cref{lmm:Dyjleh2ya/2m2/r} and \cref{lmm:Dh2Kyxi} with \(y-x_i \simeq y\), we have
    \begin{equation}
      \begin{aligned}
        | V_{ij}| & = \left| \int_{x_{j-1}}^{x_{j}}(u(y) - \Pi_hu(y)) D_h^2 K_y (x_i) dy \right| \\
               & \le C h^2 \int_{x_{j-1}}^{x_{j}} y^{\alpha/2-2/r} y^{-1-\alpha} dy                       \\
               & = C h^2 \int_{x_{j-1}}^{x_{j}} y^{-\alpha/2-2/r-1} dy
      \end{aligned}
    \end{equation}
    With \(x_{i} \simeq x_{2i} \),
    \begin{equation}
      \begin{aligned}
...
\begin{proof}
  For \(1\le i < N/2, N+1\le j\le 2N-1\), by \cref{lmm:Dyjleh2ya/2m2/r}, \cref{lmm:Dh2Kyxi} and \(y-x_i\simeq T\)
  \begin{equation*}
    \begin{aligned}
      |V_{ij}| & = \left| \int_{x_{j-1}}^{x_{j}}(u(y) - \Pi_hu(y)) D_h^2 K_y (x_i) dy \right| \\
...
\begin{lemma} \label{lmm:R1R2}
  From \eqref{eq:departR1R2},  by \cref{lmm:sumSij13}, \cref{lmm:Ri-I5-1}  and \cref{lmm:Ri-I5-2}   we get for \(i=1, 2\)
  \begin{equation}
    \begin{aligned}
...
\begin{proof}
  by \cref{lmm:Dyj1} , \cref{lmm:Dh2Kyxi}
  \begin{equation}
...
\textcolor{blue}{
  \begin{theorem} \label{Ri-I3-2lN/2}
    ...
    \begin{equation}
      |I_3| = \left| \sum_{j=k+1}^{2i-1} S_{ij} \right| \le C h^2 x_i^{-\alpha/2-2/r}
    \end{equation}
  \end{theorem}
}
...
To estimate \(S_{ij}\), we first estimate \(D_h^2 P_{i,j}^\theta(x_i)\) , but
...
\textcolor{blue}{
  \begin{lemma} \label{lmm:d2Pj-itle}
    ...
    \begin{equation}
      D_h^2 {P_{i,j}^\theta}(x_{i}) \le C h^2 \frac{|y_j^\theta - x_i|^{1-\alpha}}{\Gamma(2-\alpha)} x_i^{\alpha/2-2-2/r} h_j
    \end{equation}
    where \(y_j^\theta = \theta x_{j-1} + (1-\theta) x_{j}\)
  \end{lemma}
  \begin{proof}
...
\textcolor{blue}{
  \begin{lemma} \label{lmm:dQj-itle}
    ...
    \begin{equation}
      \begin{aligned}
        \frac{2}{h_{i} + h_{i+1}} & \left( \frac{{Q_{i,j}^\theta}(x_{i+1}) u'''(\eta_{j+1}^\theta) - {Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j}^\theta)}{h_{i+1}}\right) \\
    ...
    \begin{equation}
      \begin{aligned}
        \frac{2}{h_{i} + h_{i+1}} & \left( \frac{{Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j}^\theta) - {Q_{i,j}^\theta}(x_{i-1}) u'''(\eta_{j-1}^\theta)}{h_{i}}\right) \\
                                  & \le C h^2 \frac{|y_j^\theta - x_i|^{1-\alpha}}{\Gamma(2-\alpha)} x_i^{\alpha/2-2-2/r} h_j
      \end{aligned}
    \end{equation}
...
\textcolor{blue}{
  \begin{lemma} \label{lmm:Vijleh2xma22r}
    ...
    \begin{equation} \label{eq:estimateVij-int}
      \begin{aligned}
        S_{ij} & \le C h^2 \int_{0}^{1}  \frac{|y_j^\theta - x_i|^{1-\alpha}}{\Gamma(2-\alpha)} x_i^{\alpha/2-2-2/r} h_j d\theta \\
...
Now combine \cref{lmm:R1R2}, \cref{lmm:Ri-I1}, \cref{lmm:Ri-I2-I4-ilN/2}, \cref{Ri-I3-2lN/2}, \cref{lmm:Ri-I5-1} and \cref{lmm:Ri-I5-2}, we get \cref{thm:Ri-ilessN/2}.
...
\begin{lemma} \label{lmm:Ri-I3-i<N}
  ...
  \begin{equation}
    \begin{aligned}
      I_3^1 & = \sum_{j=k+1}^{N-1} S_{ij} \le C h^2 \int_{x_{k}}^{x_{N-1}} \frac{|y-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} x_i^{\alpha/2-2-2/r} dy                   \\
...
\begin{equation}
  I_3^3 = \sum_{j=N+2}^{2N-\lceil\frac{N}{2}\rceil} S_{ij}
\end{equation}
...
\begin{definition} \label{def:yi2j-jgN}
  ...
  \end{equation}
  \begin{equation}
    {P_{i,j}^\theta}(x) = ({h_{j-i}}(x))^3 u''({y_{j-i}^\theta}(x)) \frac{|{y_{j-i}^\theta}(x)-x|^{1-\alpha}}{\Gamma(2-\alpha)}
  \end{equation}
  \begin{equation}
    {Q_{i,j}^\theta}(x) = ({h_{j-i}}(x))^4 \frac{|{y_{j-i}^\theta}(x)-x|^{1-\alpha}}{\Gamma(2-\alpha)}
  \end{equation}
\end{definition}
...
%   \begin{aligned}
%     S_{ij} & = \frac{2}{h_{i} + h_{i+1}}  \left( \frac{1}{h_{i+1}} T_{i+1, j+1} - \left(\frac{1}{h_{i}}+\frac{1}{h_{i+1}}\right)  T_{i,j} + \frac{1}{h_{i}} T_{i-1, j-1} \right)                                                      \\
%            & = \int_{0}^{1} -\frac{\theta (1-\theta)}{2} D_h^2 P_{i,j}^\theta(x_i)  d\theta                                                                                                                                           \\
%            & \quad +  \int_{0}^1 \frac{\theta^3 (1-\theta)}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i,j}^\theta}(x_{i+1}) u'''(\eta_{j+1,1}^\theta) - {Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j,1}^\theta)}{h_{i+1}}\right)  d\theta \\
%            & \quad -  \int_{0}^1 \frac{\theta^3 (1-\theta)}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j,1}^\theta) - {Q_{i,j}^\theta}(x_{i-1}) u'''(\eta_{j-1,1}^\theta)}{h_{i}}\right)  d\theta   \\
%            & \quad -  \int_{0}^1 \frac{\theta (1-\theta)^3}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i,j}^\theta}(x_{i+1}) u'''(\eta_{j+1,2}^\theta) - {Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j,2}^\theta)}{h_{i+1}}\right)  d\theta \\
...
\begin{equation}
  D_h^2 P_{i,j}^\theta(\xi) = {P_{i,j}^\theta}''(\xi), \quad \xi\in (x_{i-1}, x_{i+1})
\end{equation}
...
\textcolor{blue}{
  ...
  \begin{lemma} \label{lmm:estimate-Pij-theta-j>N}
    ...
    \begin{equation}
      \begin{aligned}
        |D_h^2 P_{i,j}^\theta(\xi)| & \le C h_j h^2 \Big(|y_j^\theta - x_i|^{1-\alpha}                            \\
...
\textcolor{blue}{
  \begin{lemma} \label{lmm:estimate-Qij-theta-j>N}
    ...
    \begin{equation}
      \begin{aligned}
        \frac{2}{h_{i} + h_{i+1}} & \left( \frac{{Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j}^\theta) - {Q_{i,j}^\theta}(x_{i-1}) u'''(\eta_{j-1}^\theta)}{h_{i+1}}\right) \\
...
\begin{proof} \label{prf:estimate-Qij-theta-j>N}
  ...
  \begin{equation}
    \left|{Q_{i,j}^\theta}'(\xi)\right| \le C h^2 h_j^2 ((r-1)|y_j^\theta - x_i|^{1-\alpha}  + |y_j^\theta - x_i|^{-\alpha}(|2T - x_i - y_j^\theta| + h_N))
  \end{equation}
  \begin{equation}
    \left|Q_{i,j}^\theta(\xi)\right| \le C h^2 h_j^2 |y_j^\theta - x_i|^{1-\alpha}
  \end{equation}
...
\begin{lemma}
  ...
  \begin{equation}
    \begin{aligned}
      S_{ij} \le & C h^2 \int_{x_{j-1}}^{x_{j}} \Big(  |y-x_i|^{1-\alpha} \\
...
\textcolor{blue}{
  ...
  \begin{lemma} \label{thm:Ri-I5-i<N}
    ...
    \begin{equation}
      I_3^3 = \sum_{j=N+2}^{2N-\lceil\frac{N}{2}\rceil} S_{ij} \le C h^2 + C(r-1) h^2 \textcolor{red}{|T-x_{i-1}|^{1-\alpha}}
    \end{equation}
  \end{lemma}
}
\begin{proof}
...
\newpage
\(I_4, I_5\) is easy. Similar with \cref{lmm:Ri-I2-I4-ilN/2} and \cref{lmm:Ri-I5-2}, we have
...
\begin{proof}
  \begin{definition}
    ...
    \begin{gather}
      ...
      \textcolor{red}{y_j''(x) = y_j^{1-2/r}(x) \frac{r(r-1)Z_1}{h_N}}
    \end{gather}
  \end{definition}
...
\begin{lemma} \label{lmm:Dyjleh2ya/2m2/r}
  ...
  \begin{equation}
    |u(y)-\Pi_hu(y)| \le h_j^2 \max_{\xi\in [x_{j-1}, x_j]}|u''(\xi)| \le C h^2 y^{\alpha/2-2/r}, \quad \text{for } y\in (x_{j-1}, x_{j})
  \end{equation}
  symmetricly, for \(N <j \le 2N-1\), we have
  \begin{equation} \label{lmm:Dyjleh22T-y}
    |u(y) - \Pi_hu(y)| \le C h^2 (2T-y)^{\alpha/2-2/r}, \quad \text{for } y\in (x_{j-1}, x_{j})
  \end{equation}
\end{lemma}
...
\begin{lemma} \label{lmm:hilexi}
  ...
  And
  \begin{equation}
    h_i \simeq h_{i+1}, \quad x_i \simeq x_{i+1} \simeq y_i^\theta,  \quad \text{  for  } 1\le i\le 2N-1,\; \theta\in (0,1)
  \end{equation}
\end{lemma}
...
\end{lemma}
\begin{proof}
  Sinec \(y-x_{i-1}, y-x_{i}, y-x_{i+1}\) have the same sign, by \cref{lmm:Dh2simd2},
  \begin{equation*}
    \begin{aligned}
...
\begin{proof}[proof of \cref{lmm:dQj-itle}] \label{prf:dQj-itle}
  ...
  \begin{equation}
    \begin{aligned}
       & \frac{{Q_{i,j}^\theta}(x_{i+1}) u'''(\eta_{j+1}^\theta) - {Q_{i,j}^\theta}(x_{i}) u'''(\eta_{j}^\theta)}{h_{i+1}}  \\
       & \quad = \frac{Q_{i,j}^\theta(x_{i+1}) - Q_{i,j}^\theta(x_{i})}{h_{i+1}} u'''(\eta_{j+1}^\theta)
      + Q_{i,j}^\theta(x_i) \frac{u'''(\eta_{j+1}^\theta)-u'''(\eta_{j}^\theta)}{h_{i+1}}                                   \\
      %  & \quad \le {Q_{i,j}^\theta}'(\xi) C x_j^{\alpha/2-3} + Q_{i,j}^\theta(x_i) C u''''(\eta)\frac{h_i+h_{i+1}}{h_{i+1}}
    \end{aligned}
  \end{equation}
  Since mean value theorem
  \begin{equation}
    \frac{Q_{i,j}^\theta(x_{i+1}) - Q_{i,j}^\theta(x_{i})}{h_{i+1}} = {Q_{i,j}^\theta}'(\xi), \quad \xi \in (x_{i}, x_{i+1})
  \end{equation}
  From \eqref{def:Qj-itheta-jlN} and Leibniz rule, by \cref{lmm:dhj-i3le-Ch2xi1-2r} and \cref{lmm:estimatesofy-xi1-a}, we have
  \begin{equation}
    \begin{aligned}
      |{Q_{i,j}^\theta}'(\xi)| \le C h^2 \frac{|y_{j}^\theta - x_{i}|^{1-\alpha}}{\Gamma(2-\alpha)} x_{i}^{1-2/r} h_{j}^2 
    \end{aligned}
  \end{equation}
  And by \cref{def:yj-i}  and \cref{lmm:hilexi}
  \begin{equation}
    Q_{i,j}^\theta(x_i) = h_{j}^4 \frac{|y_j^\theta-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} \simeq C h^2 x_i^{2-2/r}\frac{|y_j^\theta-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} h_j^2
  \end{equation}
