...
\begin{lemma} \label{lmm:gen-prop-of-MTFs}
  For \(2 \le i \le 2N-2, 2\le j \le 2N-2\) and \(\xi \in (x_{i-1}, x_{i+1})\)
  \begin{equation}
    \xi \simeq x_i, \quad y_{i,j}(\xi)\simeq x_j, \quad 2T-y_{i,j}(\xi)\simeq 2T-x_j, \quad h_{i,j}(\xi) \simeq h_j
  \end{equation}
  % For \(1\le i, j \le 2N-1\),
  \begin{equation}
...
\begin{lemma} \label{lmm:esitmate-of-MTFs-1}
  ...
  \begin{equation}
    |h_{i,j}'(\xi)| \le C (r-1) Z_1 x_i^{1/r-1}
    \begin{cases}
      x_j^{1-2/r}   & j\le N \\
      (2T-x_j)^{1-2/r}    & j> N \\
    \end{cases}
  \end{equation}
  \begin{equation}
    \left|(y_{i,j}(\xi) - \xi)'\right| \le C x_i^{-1}
    \begin{cases}
      |x_j - x_i|      & j\le N \\
      |2T - x_j - x_i| + 2h_N & j> N \\
    \end{cases}
...
\begin{proof}
  ...
  \begin{equation} \label{eq:hN/rZ1}
    \dfrac{h_N}{r Z_1} = T^{1-1/r} \dfrac{1 - (1-h)^r}{rh} = \eta^{1-1/r}, \quad \eta \in (x_{N-1}, x_{N})
  \end{equation}
  Then
  ...
  Combine with \cref{lmm:gen-prop-of-MTFs}, \(\eta \simeq x_N\), we get the first result.
  ...
  \begin{equation} \label{eq:hN/rZ1-xi<N}
    \begin{aligned}
      ...
      & \le |x_N - x_i| + |h_N| + |h_{i+1}| \le C |x_N - x_i|
    \end{aligned}
  \end{equation}
...
\begin{lemma} \label{lmm:esitmate-of-MTFs-2}
  ...
  \begin{equation}
    ...
    \begin{cases}
      ...
      (2T - x_j)^{1-2/r} x_i^{1/r-2} x_N^{1/r}     & i < N, j > N  \\
      x_j^{1-2/r} x_N^{2/r-2}             & i = N, j < N  \\
      0   & i = N, j = N
    \end{cases}
  \end{equation}
  ...
  \begin{equation}
    ...
    \begin{cases}
      ...
      Z_1 x_i^{1/r-2} (2T - x_j)^{1-3/r}  x_N^{1/r}    & i < N, j>N+1 \\
      Z_1 x_N^{2/r-2} x_j^{1-3/r}                      & i = N, j < N  \\
      x_N^{-1}                                       & i = N, j = N
    \end{cases}
  \end{equation}
\end{lemma}
...
Then we esrimate each part of \eqref{eq:depart-Ri} from easy to hard. And We take
...
\textcolor{blue}{
  ...
  \begin{proof}
    For  \(\max\{2i+1, 4\} \le j \le N\), by \cref{def:Vij}, \cref{lmm:Dyjleh2ya/2m2/r} and \cref{lmm:Dh2Kyxi} with \(y-x_i \simeq y\), we have
    \begin{equation}
      \begin{aligned}
        | V_{ij}| 
...
\begin{proof}
  For \(1\le i < N/2, N+1\le j\le 2N-1\), by \cref{def:Vij}, \cref{lmm:Dyjleh2ya/2m2/r}, \cref{lmm:Dh2Kyxi} and \(y-x_i\simeq T\)
  \begin{equation*}
    \begin{aligned}
      |V_{ij}| 
      % & = \left| \int_{x_{j-1}}^{x_{j}}(u(y) - \Pi_hu(y)) D_h^2 K_y (x_i) dy \right| \\
...
\textcolor{blue}{
  ...
  \begin{proof}
    ...
    \end{equation}
    From \eqref{def:Pj-itheta-jlN}, using Leibniz formula and chain rule, and 
    \cref{lmm:gen-prop-of-MTFs}, 
    \cref{lmm:esitmate-of-MTFs-1}, 
    \cref{lmm:esitmate-of-MTFs-2}, 
    \cref{cor:regularity-u},
    \cref{lmm:hilexi}
    and \(x_j \simeq x_i\) we get the result.
  \end{proof}
}
  
  \textcolor{blue}{
    \begin{lemma} \label{lmm:dQj-itle}
      There exists a constant \(C=C(T, \alpha, r, \|u\|_{\beta+\alpha}^{(-\alpha/2)})\) such that for \(3\le i \le N\). \\
      ...
      \begin{equation}
        \begin{aligned}
          ...
          \le C h^2 \frac{|y_j^\theta - x_i|^{1-\alpha}}{\Gamma(2-\alpha)} x_i^{\alpha/2-2/r+2-l} h_j^{l-2}
        \end{aligned}
      \end{equation}
    ...
    \begin{proof} \label{prf:dQj-itle}
      ...
      \begin{equation}
        \begin{aligned}
          |{Q_{i,j;l}^\theta}'(\xi)| \le C h^2 x_{i}^{1-2/r} \frac{|y_{j}^\theta - x_{i}|^{1-\alpha}}{\Gamma(2-\alpha)} h_{j}^{l-2} 
        \end{aligned}
      \end{equation}
      % And
      \begin{equation}
        Q_{i,j;l}^\theta(x_i) = h_{j}^l \frac{|y_j^\theta-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} \simeq C h^2 x_i^{2-2/r}\frac{|y_j^\theta-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} h_j^{l-2}
      \end{equation}
      With \(\eta_{j}^\theta \in (x_{j-1}, x_{j})\)
      \begin{equation*}
        u^{(l-1)}(\eta_{j+1}^\theta) \le C (\eta_{j+1}^\theta)^{\alpha/2-l+1} \simeq x_j^{\alpha/2-l+1} \simeq x_i^{\alpha/2-l+1}
      \end{equation*}
      and
      \begin{equation*}
        \begin{aligned}
          ...
          & \le C \eta^{\alpha/2-l} \frac{x_{j+1}-x_{j-1}}{h_{i+1}}
          = C \eta^{\alpha/2-l} \frac{h_{j+1}+h_{j}}{h_{i+1}} \\
          & \simeq x_j^{\alpha/2-l} \simeq x_i^{\alpha/2-l}
        \end{aligned}
      \end{equation*}
      % With 
      % \begin{equation}
      %   \begin{aligned}
      %      & \left|\frac{{Q_{i,j;l}^\theta}(x_{i+1}) u'''(\eta_{j+1}^\theta) - {Q_{i,j;l}^\theta}(x_{i}) u'''(\eta_{j}^\theta)}{h_{i+1}} \right|
      ...
      %      \le C h^2 \frac{|y_j^\theta-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} x_i^{\alpha/2-2-2/r} h_j^2
      %   \end{aligned}
      % \end{equation}
      with \(h_j \simeq h_{i}\), we get the first term.
      % subsitute into the inequality above, we get the goal
      % \begin{equation}
      %   \begin{aligned}
      %     \frac{2}{h_{i} + h_{i+1}} & \left( \frac{{Q_{i,j;l}^\theta}(x_{i+1}) u'''(\eta_{j+1}^\theta) - {Q_{i,j;l}^\theta}(x_{i}) u'''(\eta_{j}^\theta)}{h_{i+1}}\right) \\
...
\begin{proof}
  By \cref{lmm:Vijleh2xma22r}, while \(x_k \simeq x_i \simeq x_{\min\{2i-1, N-1\}}\), we have
  \begin{equation}
  \begin{aligned}
    \sum_{k+1}^{\min\{2i-1, N-1\}} |S_{ij}|
    &\le C h^2 \int_{x_{k}}^{x_{\min\{2i-1, N-1\}}} \frac{|y - x_i|^{1-\alpha}}{\Gamma(2-\alpha)} x_i^{\alpha/2-2-2/r} dy    \\
...
Now we study \(I_2, I_4\).
  ...
  \begin{proof}
    ...
    \end{equation}
    From \cref{def:Tij}, \cref{lmm:Dyj} and normalizzation, we have
    \begin{equation}
      \begin{aligned}
...
For \(N/2 \le i < N \), we take \(m=2N - \lceil\frac{N}{2}\rceil+1\).
...
We have estimated \(I_3^1\) in \cref{Ri-I3-2lN/2}.
...
\textcolor{blue}{
  ...
  \cref{lmm:esitmate-of-MTFs-1}, 
  \cref{lmm:esitmate-of-MTFs-2},
  \cref{lmm:hilexi}, using Leibniz formula, we have
...
\textcolor{blue}{
  \begin{lemma} \label{lmm:estimate-Qij-theta-j>N}
    ...
    \begin{equation}
      \begin{aligned}
        \frac{2}{h_{i} + h_{i+1}} & \left( \frac{{Q_{i,j;l}^\theta}(x_{i}) u'''(\eta_{j}^\theta) - {Q_{i,j;l}^\theta}(x_{i-1}) u'''(\eta_{j-1}^\theta)}{h_{i+1}}\right) \\
...
\begin{proof} \label{prf:estimate-Qij-theta-j>N}
  ...
  \begin{equation}
    \left|{Q_{i,j;l}^\theta}'(\xi)\right| \le C h^2 h_j^2 ((r-1)|y_j^\theta - x_i|^{1-\alpha}  + |y_j^\theta - x_i|^{-\alpha}(|2T - x_i - y_j^\theta| + h_N))
  \end{equation}
  \begin{equation}
    \left|Q_{i,j;l}^\theta(\xi)\right| \le C h^2 h_j^2 |y_j^\theta - x_i|^{1-\alpha}
  \end{equation}
