...
\textcolor{orange}{
  ...
  \begin{theorem}[Truncation Error] \label{thm:truncation-error}
    ...
    $C_1, C_2=C_1(T,\alpha, r, \|u\|_{\beta+\alpha}^{(-\alpha/2)}, \|f\|_{C^2(\Omega)}), C_2(T,\alpha, r, \|f\|_{\beta}^{(\alpha/2)})$, such that
    ...
    \begin{equation} \label{eq:truncation-error}
      \begin{aligned}
        ...
        \le & C_1  h^{\min\{\frac{r\alpha}{2}, 2\}}(x_i^{-\alpha}+(2T-x_i)^{-\alpha}) \\
            & + C_2 h^2
        \begin{cases}
  ...
  \begin{theorem} \label{thm:Ri-ilessN/2}
    ...
    \begin{equation}
      R_i \le \begin{cases}
        ...
        C h^{r\alpha/2} x_i^{-1-\alpha},        & \alpha/2 - 2/r < 0
      \end{cases}
    \end{equation}
  \end{theorem}
}
\textcolor{orange}{
  \begin{theorem} \label{thm:Ri-N/2le-i-lN}
...
\begin{proof}
  ...
  \begin{equation}
    \begin{aligned}
      S_{ij} & = \int_{x_{j-1}}^{x_{j}}(u(y) - u_h(y)) D_h^2 \left(\frac{|\cdot-y|^{1-\alpha}}{\Gamma(2-\alpha)}\right)(x_i) dy \\
    ...
    \end{aligned}
  \end{equation}
  And by \cref{lmm:Dyj1} , \cref{lmm:Dh2xmy1malecx1ma}
  \begin{equation}
...
\begin{lemma} \label{lmm:Tijexpand}
  Denote \(y_j^\theta = \theta x_{j-1} + (1-\theta) x_j, \theta\in [0,1]\), by \cref{lmm:Dyj}
  \begin{equation}
    \begin{aligned}
      T_{ij} & = \int_{x_{j-1}}^{x_{j}} (u(y) - u_h(y)) \frac{|y-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} dy                                                                                              \\
             & = \int_{x_{j-1}}^{x_{j}}
      -\frac{\theta (1-\theta)}{2} h_j^2 u''(y_j^\theta)\frac{|y_j^\theta-x_i|^{1-\alpha}}{\Gamma(2-\alpha)}                                                                                        \\
             & \quad + \frac{\theta (1-\theta)}{3!} h_j^3 \frac{|y_j^\theta-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} (\theta^2 u'''(\eta_{j1}^\theta) - (1-\theta)^2 u'''(\eta_{j2}^\theta)) d y_j^\theta \\
             & = \int_{0}^{1}
...
\begin{definition} \label{def:yj-i}
  ...
  \begin{equation} \label{def:xi2yj-jlNt}
    {y_{j-i}^{\theta}}(x) = \theta {y_{j-1-i}}(x) + (1-\theta) {y_{j-i}}(x)
  \end{equation}
  ...
  \begin{equation}
    \begin{aligned}
      T_{ij} & = \int_{0}^{1} -\frac{\theta (1-\theta)}{2} {P_{j-i}^\theta}(x_i) d\theta                                                                                               \\
             & + \int_{0}^{1} \frac{\theta (1-\theta)}{3!}( \theta^2 {Q_{j-i}^\theta}(x_i) u'''(\eta_{j1}^\theta) - (1-\theta)^2 {Q_{j-i}^\theta}(x_i) u'''(\eta_{j2}^\theta)) d\theta
    \end{aligned}
  \end{equation}
\end{lemma}
...
\begin{lemma} \label{lmm:Vij-int}
  For \(3 \le i \le N-1\), \( 3 \le j \le N-1\),
  \begin{equation}
    \begin{aligned}
      V_{ij} & = \frac{2}{h_{i} + h_{i+1}}  \left( \frac{1}{h_{i+1}} T_{i+1, j+1} - \left(\frac{1}{h_{i}}+\frac{1}{h_{i+1}}\right)  T_{i,j} + \frac{1}{h_{i}} T_{i-1, j-1} \right)                                                      \\
...
\(h_{j-i}^3\), \(u''(y_{j-i}^{\theta}(x)) \) and \(\frac{|{y_{j-i}^\theta}(x)-x|^{1-\alpha}}{\Gamma(2-\alpha)}\) separately.
    ...
    \begin{equation}
      \begin{aligned}
        \frac{1}{h_{i+1}} (T_{i+1, k} -  T_{i, k})
         ...
         & \le \int_{x_{k-1}}^{x_k} h_j^2 u''(\eta)\; \frac{|\xi-y|^{-\alpha}}{\Gamma(1-\alpha)} dy                            \\
         & \le C h_j h^2 x_j^{2-2/r} x_{k-1}^{\alpha/2-2} |x_i-x_{k}|^{-\alpha}                                                \\
         & \le C h_j h^2 x_i^{-\alpha/2-2/r}
      \end{aligned}
    \end{equation}
    Thus,
    \begin{equation}
    ...
    \begin{equation}
      \begin{aligned}
        \frac{1}{h_{i+1}} (T_{i+1, k+1} -  T_{i, k})
         ...
         & \quad + \int_{0}^1 \frac{\theta^3 (1-\theta)}{3!} \frac{{Q_{k-i}^\theta}(x_{i+1}) u'''(\eta_{j+1,1}^\theta) - {Q_{k-i}^\theta}(x_{i}) u'''(\eta_{j,1}^\theta)}{h_{i+1}}  d\theta   \\
         & \quad -  \int_{0}^1 \frac{\theta (1-\theta)^3}{3!}  \frac{{Q_{k-i}^\theta}(x_{i+1}) u'''(\eta_{j+1,2}^\theta) - {Q_{k-i}^\theta}(x_{i}) u'''(\eta_{j,2}^\theta)}{h_{i+1}}  d\theta
      \end{aligned}
    \end{equation}
    And by \cref{lmm:DhPj-it}
    \begin{equation}
      \begin{aligned}
        \frac{P_{k-i}^\theta(x_{i+1}) - P_{k-i}^\theta(x_i)}{h_{i+1}} \le C h^2 x_i^{-\alpha/2-2/r} h_j
      \end{aligned}
    \end{equation}
...
Combine with \(i=1, 2\), we get for \(1\le i\le N/2\)
\begin{equation}
  R_i \le \begin{cases}
    C h^2 x_i^{-\alpha/2-2/r} ,              & r\alpha/2 + r -2 > 0 \\
    C h^2 (x_i^{-1-\alpha} \ln(i) + \ln(N)), & r\alpha/2 + r -2 = 0 \\
...
\subsection{Proof of \cref{thm:Ri-N/2le-i-lN}}
...
\begin{definition} \label{def:yi2j-jgN}
  ...
  \end{gather}
  \begin{equation}
    y_{j-i}^\theta(x) = \theta y_{j-i-1}(x) + (1-\theta) y_{j-i}(x)
  \end{equation}
  \begin{equation}
    h_{j-i}(x) = y_{j-i}(x) - y_{j-i-1}(x)
  \end{equation}
...
\textcolor{red}{
  \begin{theorem}
    There exists a constant \(C=C(T, \alpha, r, \|u\|_{\beta+\alpha}^{(-\alpha/2)})\) such that, for \(N/2\le i< N-1\)
    \begin{equation}
      \begin{aligned}
...
\begin{proof}
  ...
  and
  \begin{equation}
    y_{i\to N+1}(x) = 2T - (Z_{N-1+i} - x^{1/r})^r, \quad Z_{N-1+i} = T^{1/r}\frac{N-1+i}{N}
  \end{equation}
  Thus,
  \begin{gather*}
    ...
    y_{i\to N-1}(x_{i-1}) = x_{N}, \quad  y_{i\to N}(x_{i}) = x_{N+1}, \quad y_{i\to N}(x_{i+1}) = x_{N+2}
  \end{gather*}
  Then, define
  \begin{gather}
    y_{i\to N}^\theta(x) = \theta y_{i\to N-1}(x) + (1-\theta) y_{i\to N}(x)   \\
    y_{i\to N+1}^\theta(x) = \theta y_{i\to N}(x) + (1-\theta) y_{i\to N+1}(x)
  \end{gather}
  \begin{gather}
    h_{i\to N}(x) = y_{i\to N}(x) - y_{i\to N-1}(x)   \\
    h_{i\to N+1}(x) = y_{i\to N+1}(x) - y_{i\to N}(x)
  \end{gather}
  We have
  \begin{gather}
    ...
    {y_{i\to N+1}}'(x) = (2T-y_{i\to N+1}(x))^{1-1/r} x^{1/r-1}  \\
    {y_{i\to N+1}}''(x) = \frac{1-r}{r} (2T-y_{i\to N+1}(x))^{1-2/r} x^{1/r-2} Z_{N-1+i}
  \end{gather}
  \begin{gather}
    P_{i\to N}^\theta(x) = (h_{i\to N}(x))^3 \frac{|y_{i\to N}^\theta(x)-x|^{1-\alpha}}{\Gamma(2-\alpha)} u''(y_{i\to N}^\theta(x)) \\
    P_{i\to N+1}^\theta(x) = (h_{i\to N+1}(x))^3 \frac{|y_{i\to N+1}^\theta(x)-x|^{1-\alpha}}{\Gamma(2-\alpha)} u''(y_{i\to N+1}^\theta(x))
  \end{gather}

  \begin{gather}
    Q_{i\to N}^\theta(x) = (h_{i\to N}(x))^4 \frac{|y_{i\to N}^\theta(x)-x|^{1-\alpha}}{\Gamma(2-\alpha)}  \\
    Q_{i\to N+1}^\theta(x) = (h_{i\to N+1}(x))^4 \frac{|y_{i\to N+1}^\theta(x)-x|^{1-\alpha}}{\Gamma(2-\alpha)}
  \end{gather}
  ...
  \begin{equation}
    \begin{aligned}
      T_{i+l, N+1+l} & = \int_{0}^{1} -\frac{\theta (1-\theta)}{2} {P_{i\to N+1}^\theta}(x_{i+l}) d\theta                                                                                     \\
                     & + \int_{0}^{1} \frac{\theta (1-\theta)}{3!}{Q_{i\to N+1}^\theta}(x_{i+l})( \theta^2  u'''(\eta_{N+1+l, 1}^\theta) - (1-\theta)^2 u'''(\eta_{N+1+l, 2}^\theta)) d\theta
    \end{aligned}
  \end{equation}
  So we have
  \begin{equation}
    \begin{aligned}
      ...
      % & \quad +  \int_{0}^1 \frac{\theta^3 (1-\theta)}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i\to N+1}^\theta}(x_{i+1}) u'''(\eta_{N+2,1}^\theta) - {Q_{i\to N+1}^\theta}(x_{i}) u'''(\eta_{N+1,1}^\theta)}{h_{i+1}}\right)  d\theta \\
      % & \quad -  \int_{0}^1 \frac{\theta^3 (1-\theta)}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i\to N+1}^\theta}(x_{i}) u'''(\eta_{N+1,1}^\theta) - {Q_{i\to N+1}^\theta}(x_{i-1}) u'''(\eta_{N,1}^\theta)}{h_{i}}\right)  d\theta   \\
      % & \quad -  \int_{0}^1 \frac{\theta (1-\theta)^3}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i\to N+1}^\theta}(x_{i+1}) u'''(\eta_{N+2,2}^\theta) - {Q_{i\to N+1}^\theta}(x_{i}) u'''(\eta_{N+1,2}^\theta)}{h_{i+1}}\right)  d\theta \\
      % & \quad +  \int_{0}^1 \frac{\theta (1-\theta)^3}{3!} \frac{2}{h_{i} + h_{i+1}}\left( \frac{{Q_{i\to N+1}^\theta}(x_{i}) u'''(\eta_{N+1,2}^\theta) - {Q_{i\to N+1}^\theta}(x_{i-1}) u'''(\eta_{N,2}^\theta)}{h_{i}}\right)  d\theta
    \end{aligned}
  \end{equation}
  \(N+1\) is similar.

  We estimate \(D_h^2 P_{i\to N}^\theta(x_i)={P_{i\to N}^\theta}''(\xi) , \xi \in [x_{i-1}, x_{i+1}]\),


  \textcolor{orange}{
    \begin{lemma}
      \begin{gather}
        h_{i\to N}^3(\xi) \le C h_N^3 \le C h^3  \\
        h_{i\to N+1}^3(\xi) \le C h_N^3 \le C h^3  \\
        (h_{i\to N}^3(\xi))' \le C (r-1) h_N^2 h \le C (r-1) h^3  \\
        (h_{i\to N+1}^3(\xi))' \le C (r-1) h_N^2 h \le C (r-1) h^3  \\
        (h_{i\to N}^3(\xi))''  \le C (r-1) h^2     \\
        (h_{i\to N+1}^3(\xi))''  \le C (r-1) h^2
      \end{gather}
    \end{lemma}
  }
  \begin{proof}
    \begin{equation}
      h_{i\to N}(\xi) \le 2h_N, \quad h_{i\to N+1}(\xi) \le 2h_N
    \end{equation}
    \begin{equation}
      \begin{aligned}
        (h_{i\to N}^l(\xi))' & = l h_{i\to N}^{l-1}(\xi) ({y_{i\to N}}'(\xi) - {y_{i\to N-1}}'(\xi))                           \\
                             & = l h_{i\to N}^{l-1}(\xi) x_i^{1/r-1} (\frac{1}{r}\frac{h_N}{Z_1}  - y_{i\to N-1}^{1-1/r}(\xi)) \\
      \end{aligned}
    \end{equation}
    while
    \begin{equation}
      \begin{aligned}
        |\frac{1}{r}\frac{h_N}{Z_1}  - y_{i\to N-1}^{1-1/r}(\xi)|
    ...
    \begin{equation}
      \begin{aligned}
        (h_{i\to N}^l(\xi))' & \le C(r-1) h_N^{l-1} x_i^{1/r-1} h
      \end{aligned}
    \end{equation}

    \begin{equation}
      \begin{aligned}
        (h_{i\to N+1}^l(\xi))' & = l h_{i\to N+1}^{l-1}(\xi) ({y_{i\to N+1}}'(\xi) - {y_{i\to N}}'(\xi))                                \\
                               & = l h_{i\to N+1}^{l-1}(\xi) x_i^{1/r-1} ( (2T-y_{i\to N+1}(\xi))^{1-1/r}  -\frac{1}{r}\frac{h_N}{Z_1}) \\
      \end{aligned}
    \end{equation}
    Similarly,
    \begin{equation}
      \begin{aligned}
        | (2T-y_{i\to N+1})^{1-1/r} - \frac{1}{r}\frac{h_N}{Z_1} |
    ...
    \begin{equation}
      \begin{aligned}
        (h_{i\to N}^3(\xi))'' & = 3 h_{i\to N}^2(\xi) {h_{i\to N}}''(\xi) + 6 h_{i\to N}(\xi) ({h_{i\to N}}'(\xi))^2                                                     \\
                              & \le C h_N^2 \frac{1-r}{r} x_i^{1/r-2} (\frac{1}{r}\frac{h_N}{Z_1}  - y_{i\to N-1}^{1-2/r}(\xi)Z_{N-1-i}) + C h_N (r-1)^2 h^2 x_i^{2/r-2}
      \end{aligned}
    \end{equation}

    \begin{equation*}
      \begin{aligned}
        |\frac{h_N}{rZ_1}- y_{i\to N-1}^{1-2/r}(\xi)Z_{N-1-i}| \le T^{1-1/r} + C x_N^{1-2/r} x_N^{1/r} = C T^{1-1/r}
      \end{aligned}
    \end{equation*}
    So
    \begin{equation}
      \begin{aligned}
        (h_{i\to N}^3(\xi))''
         ...
         & \le C (r-1) h_N^2 x_i^{1/r-1}
      \end{aligned}
    \end{equation}

    \(h_{i\to N+1}^3(\xi)\) is similar.
  \end{proof}
  ...
  \begin{lemma}
    \begin{gather}
      u''(y_{i\to N}^\theta(\xi)) \le C x_{N-2}^{-\alpha/2-2} \le C\\
      (u''(y_{i\to N}^\theta(\xi)))' \le C \\
      (u''(y_{i\to N}^\theta(\xi)))'' \le C
    \end{gather}
  \end{lemma}
  \begin{proof}
    \begin{equation}
      \begin{aligned}
        (u''(y_{i\to N}^\theta(\xi)))' & = u'''(y_{i\to N}^\theta(\xi)) {y_{i\to N}^\theta}'(\xi)                              \\
                                       & \le C (\theta {y_{i\to N-1}}'(\xi)  + (1-\theta) {y_{i\to N}}'(\xi))                  \\
                                       & \le C x_i^{1/r-1} (\theta y_{i\to N-1}^{1-1/r}(\xi)  + (1-\theta) \frac{h_N}{rZ_1}  ) \\
    ...
    \begin{equation}
      \begin{aligned}
        (u''(y_{i\to N}^\theta(\xi)))'' & = u''''(y_{i\to N}^\theta(\xi)) ({y_{i\to N}^\theta}'(\xi))^2 + u'''(y_{i\to N}^\theta(\xi)) {y_{i\to N}^\theta}''(\xi)     \\
  ...
  \begin{lemma}
    ...
    \begin{gather}
      |y_{i\to N}^\theta(\xi) - \xi|^{1-\alpha} \le C |y_N^\theta - x_i|^{1-\alpha} \\
      (|y_{i\to N}^\theta(\xi) - \xi|^{1-\alpha})' \le C |y_N^\theta - x_i|^{1-\alpha} \\
      (|y_{i\to N}^\theta(\xi) - \xi|^{1-\alpha})'' \le C(r-1) |y_N^\theta - x_i|^{-\alpha} + |y_N^\theta - x_i|^{1-\alpha}
    \end{gather}
  \end{lemma}
  \begin{proof}
    \begin{equation}
      \begin{aligned}
        (y_{i\to N}^\theta(\xi) - \xi)'
         & = (\theta (y_{i\to N-1}(\xi) - \xi) + (1-\theta) (y_{i\to N}(\xi) - \xi))'                                                 \\
         & = \theta ({y_{i\to N-1}}'(\xi) - 1 ) + (1-\theta) ({y_{i\to N}}'(\xi) - 1)                                                 \\
         & = \theta \xi^{1/r-1} (y_{i\to N-1}^{1-1/r}(\xi) - \xi^{1-1/r} ) + (1-\theta) \xi^{1/r-1} (\frac{h_N}{rZ_1} - \xi^{1-1/r} ) \\
      \end{aligned}
    \end{equation}
    \begin{equation}
      \begin{aligned}
        (y_{i\to N}^\theta(\xi) - \xi)''
         & = \theta ({y_{i\to N-1}}''(\xi)) + (1-\theta) ( {y_{i\to N}}''(\xi) )                                           \\
         & = \frac{1-r}{r} \xi^{1/r-2}(\theta y_{i\to N-1}^{1-2/r}(\xi) Z_{N-1-i} + (1-\theta) \frac{h_N}{r Z_1}  )  \le 0
      \end{aligned}
    \end{equation}
    And
    \begin{equation}
      |(y_{i\to N}^\theta(\xi) - \xi)''| \le C (r-1) \xi^{1/r-2} T^{1-1/r}
    \end{equation}
    ...
    We have known
    \begin{equation}
      C |x_{N-1} - x_i| \le |y_{i\to N-1}(\xi) - \xi| \le C |x_{N-1} - x_i|
    \end{equation}
    If \(\xi\le x_{N-1}\), then \((y_{i\to N}(\xi) - \xi)'\ge 0\), so
    \begin{equation}
      C|x_{N}-x_{i}|  \le |x_{N-1} - x_{i-1}| \le |y_{i\to N}^\theta(\xi) - \xi| \le |x_{N+1} - x_{i+1}| \le C |x_{N} - x_{i}|
    \end{equation}
    If \(i=N-1\) and \(\xi \in [x_{N-1}, x_{N}]\), then \(y_{i\to N}(\xi) - \xi\) is concave, bigger than its two neighboring points, which are equal to \(h_N\), so
    \begin{equation}
      h_N = |x_{N}-x_{N-1}| \le |y_{i\to N}(\xi) - \xi| \le |x_{N+1} - x_{N-1}| = 2h_N
    \end{equation}
    So we have
    \begin{equation}
      |y_{i\to N}^\theta(\xi) - \xi|^{1-\alpha} \le C |y_N^\theta - x_i|^{1-\alpha}
    ...
    While
    \begin{equation}
      y_{i\to N-1}^{1-1/r}(\xi) - \xi^{1-1/r} \le (y_{i\to N-1}(\xi) - \xi)\xi^{-1/r}
    \end{equation}
    and
    \begin{equation}
      \begin{aligned}
    ...
    So we have
    \begin{equation}
      (y_{i\to N}^\theta(\xi) - \xi)' \le C |y_N^\theta - x_i|
    \end{equation}
    \begin{equation}
      \begin{aligned}
        (|y_{i\to N}^\theta(\xi) - \xi|^{1-\alpha})' & = |y_{i\to N}^\theta(\xi) - \xi|^{-\alpha} (y_{i\to N}^\theta(\xi) - \xi)' \\
    ...
    \begin{equation}
      \begin{aligned}
        (|y_{i\to N}^\theta(\xi) - \xi|^{1-\alpha})'' & = (1-\alpha)|y_{i\to N}^\theta(\xi) - \xi|^{-\alpha} (y_{i\to N}^\theta(\xi) - \xi)''                   \\
                                                      & \quad + \alpha(\alpha-1) |y_{i\to N}^\theta(\xi) - \xi|^{-1-\alpha} ((y_{i\to N}^\theta(\xi) - \xi)')^2 \\
    ...
    % \begin{equation}
    %   \begin{aligned}
    %     (|y_{i\to N}^\theta(\xi) - \xi|^{1-\alpha})'' &\le C(r-1)|
    %   \end{aligned}
    % \end{equation}
  ...
  \begin{lemma} \label{lmm:d2Pi2N}
    \begin{equation}
      \begin{aligned}
        D_h^2 P_{i\to N}^\theta(x_i) & = {P_{i\to N}^\theta}''(\xi) \quad \xi \in [x_{i-1}, x_{i+1}]                                                          \\
  ...
  \begin{lemma}
    \begin{equation}
      \begin{aligned}
        \frac{2}{h_{i} + h_{i+1}} & \left( \frac{{Q_{i\to N}^\theta}(x_{i+1}) u'''(\eta_{N+1}^\theta) - {Q_{i\to N}^\theta}(x_{i}) u'''(\eta_{N}^\theta)}{h_{i+1}}\right) \\
  ...
  \begin{equation}
    \begin{aligned}
      ...
      & = \frac{2}{h_{N-1}+h_{N}} \frac{1}{h_{N-1}}\left(  h_{N-1}^{4-\alpha}u''(y_{N-1}^\theta) -  h_N^{4-\alpha} u''(y_N^\theta) \right)  \\
        & \quad - \frac{2}{h_{N-1}+h_{N}} \frac{1}{h_N}\left(  h_N^{4-\alpha} u''(y_N^\theta) -  h_{N+1}^{4-\alpha} u''(y_{N+1}^\theta)    \right)  \\
...
\begin{lemma} \label{lmm:Dyjleh2ya/2m2/r}
  There is a constant \(C=C(T, \alpha, r, \|u\|_{\beta+\alpha}^{(-\alpha/2)})\) for \(2\le j \le N\), if \(y\in [x_{j-1}, x_{j}]\),
  \begin{equation}
...
\begin{lemma} \label{lmm:Dh2ymx1malecy1ma}
  ...
  \(\max\{2i+1, i+3\} \le j \le 2N\) and \(y\in [x_{j-1}, x_{j}]\), we have
  \begin{equation}
    D_h^2(\frac{|y-\cdot|^{1-\alpha}}{\Gamma(2-\alpha)})(x_i) \le C \frac{y^{-1-\alpha}}{\Gamma(-\alpha)}
  \end{equation}
\end{lemma}
\begin{proof}
...
\begin{lemma} \label{lmm:Dh2xmy1malecx1ma}
  There is a constant \(C=C(\alpha, r)\) such that for all \(3 \le i < N/2, k = \lceil \frac{i}{2} \rceil\),
  \(1 \le j \le k-1\) and \(y\in [x_{j-1}, x_{j}]\), we have
  \begin{equation}
    D_h^2(\frac{|\cdot-y|^{1-\alpha}}{\Gamma(2-\alpha)})(x_i) \le C \frac{x_i^{-1-\alpha}}{\Gamma(-\alpha)}
  \end{equation}
\end{lemma}
\begin{proof}
  ...
  \begin{equation*}
    \begin{aligned}
      D_h^2(\frac{|\cdot-y|^{1-\alpha}}{\Gamma(2-\alpha)})(x_i) & = \frac{|\xi-y|^{-1-\alpha}}{\Gamma(-\alpha)}, \quad \xi \in [x_{i-1}, x_{i+1}]                                    \\
  ...
  \begin{equation}
    \begin{aligned}
      (h_{j-i}^3(\xi))'' & = 6 h_{j-i}(\xi)(y_{j-i}'(\xi)-y_{j-i-1}'(\xi))^2 + 3 h_{j-i}^2(\xi)( y_{j-i}''(\xi)-y_{j-i-1}''(\xi))               \\
                         & = 6 h_{j-i}(\xi) \xi^{1/r-1} (y_{j-i}^{1-1/r}(\xi)- y_{j-i-1}^{1-1/r}(\xi))                                          \\
  ...
  \begin{equation}
    \begin{aligned}
      y_{j-i}^{1-2/r}(\xi)-y_{j-i-1}^{1-2/r}(\xi)
       ...
       & = (r-2) T^{-r} h y_{j-i-\gamma}^{1-3/r}(\xi)                  \\
...
\begin{proof}[proof of \cref{lmm:estimatesofu2yj-itheta}] \label{prf:estimatesofu2yj-itheta}
  ...
  Since
  \begin{equation*}
    x_{j-2} \le y_{j-i-1}(x_{i-1}) \le  y_{j-i}^\theta(\xi) \le y_{j-i-1}^\theta(x_{i+1}) \le x_{j+1}
  \end{equation*}
