  ...
  \begin{theorem} \label{thm:Ri-ilessN/2}
    ...
    \begin{equation}
      R_i \le \begin{cases}
        ...
        C h^{r\alpha/2} x_i^{-1-\alpha},        & \alpha/2 - 2/r + 1 < 0
      \end{cases}
    \end{equation}
  \end{theorem}
}
...
\begin{proof}
  ...
  Thus,
  \begin{equation}
    \frac{2}{h_i + h_{i+1}} \frac{1}{h_{i}} (T_{i-1, l} -  T_{i, l}) \le C h_l^3
  \end{equation}
  For
  \begin{equation}
    \begin{aligned}
...
Now we can conclude a part of  the theorem \cref{thm:Ri-N/2le-i-lN}  at the beginning of this section.
...
\cref{thm:Ri-I6-ileN}
\cref{lmm:Ri-I7-ileN}
, we have there exists a constant \(C=C(T, \alpha, r, \|u\|_{\beta+\alpha}^{(-\alpha/2)})\) such that
for \(N/2\le i< N\),
\begin{equation}
  \begin{aligned}
    R_i &= \sum_{j=1}^7 I_j  \\
