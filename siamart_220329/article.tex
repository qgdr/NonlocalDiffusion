...
\textcolor{orange}{
  ...
  \begin{theorem}[Truncation Error] \label{thm:truncation-error}
    If $f \in C^2(\Omega)$ and $\alpha\in(1,2)$, and $u(x)$ is a solution of the equation \eqref{eq:equation}, then there exists  constants
...
\begin{proof}
  \begin{definition}
    ...
    \begin{equation}
      y_{j}(x) = (\frac{Z_1}{h_N}  (x-x_N) + Z_j)^r
    \end{equation}
    We can see that is the inverse of the function \(y_{i\to N}(x)\) defined in \cref{thm:Ri-I4-i<N}.
    \begin{gather}
