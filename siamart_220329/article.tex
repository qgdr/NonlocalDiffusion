...
\textcolor{orange}{suppose \(f\in C^\beta(\Omega)\cap L^\infty(\Omega), \beta>4, \|f\|_{\beta}^{\alpha/2}<\infty\)}
...
\textcolor{blue}{
  \begin{theorem} \label{lmm:trunerror2}
    ...
    \begin{equation}
      -\kappa_\alpha (D_h^2 - \frac{d^2}{dx^2}) I^{2-\alpha} (x_i) \le C h^2 (x_i^{-\alpha/2-2/r} + (2T-x_i)^{-\alpha/2-2/r})
    \end{equation}
  \end{theorem}
  \begin{proof}
    ...
    1.
    \begin{equation}
      \left| \frac{h_{i+1}-h_{i}}{3} f'(x_i) \right| \le \frac{\|f\|_{\beta}^{(\alpha/2)}}{3} 2^{|r-2|}r(r-1)T^{2/r} h^2 \begin{cases}
    ...
    \end{equation}
    2.
    See \cref{prf:trucerr2d2f}, there is a constant \(C\) such that
    \begin{equation}
      \begin{aligned}
        \frac{1}{4!} \frac{2}{h_i + h_{i+1}} & (h_i^3 f''(\eta_1) + h_{i+1}^3 f''(\eta_2)) \\
        &\le C h^2 (x_i^{-\alpha/2-2/r} + (2T-x_i)^{-\alpha/2-2/r})
      \end{aligned}
    \end{equation}
  \end{proof}
}
...
\begin{proof}[Additional proof of \cref{lmm:trunerror2}] \label{prf:trucerr2d2f}
  ...
  \begin{equation*}
    \frac{2}{h_i + h_{i+1}} (h_i^3 f''(\eta_1) + h_{i+1}^3 f''(\eta_2)) \le C h^2 x_i^{-\alpha/2-2/r}, \quad 1\le i\le 2N-1
  \end{equation*}
\end{proof}
