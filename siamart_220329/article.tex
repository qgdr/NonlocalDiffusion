...
The Hölder spaces \(C^{k, \beta'}(\Omega)\) are defined as the subspaces of \(C^k(\Omega)\) consisting of functions whose \(k\)-th order partial derivatives are locally Hölder continuous \textcolor{red}{cite} with exponent \(\beta'\) in \(\Omega\),
...
\begin{proof}
  ...
  Thus,
  \begin{gather*}
    y_{i\to N-1}(x_{i-1}) = x_{N-2}, \quad  y_{i\to N-1}(x_{i}) = x_{N-1}, \quad y_{i\to N-1}(x_{i+1}) = x_{N} \\
    y_{i\to N}(x_{i-1}) = x_{N-1}, \quad  y_{i\to N}(x_{i}) = x_{N}, \quad y_{i\to N}(x_{i+1}) = x_{N+1} \\
    y_{i\to N+1}(x_{i-1}) = x_{N}, \quad  y_{i\to N+1}(x_{i}) = x_{N+1}, \quad y_{i\to N+1}(x_{i+1}) = x_{N+2}
  \end{gather*}
  Then, define
  \begin{gather}
