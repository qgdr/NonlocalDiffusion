...
\begin{equation} \label{def:Ih}
  I_h^{2-\alpha}u(x_i) := I^{2-\alpha}\Pi_hu(x_i) = \frac{1}{\Gamma(2-\alpha)}\int_{\Omega} |x_i-y|^{1-\alpha} \Pi_hu(y) dy
\end{equation}
And now, we can approximate the operator \eqref{def:operator} at $x_i$ with
\begin{equation} \label{def:Dhalpha}
  \begin{aligned}
    D_h^{\alpha} u(x_i) & := D_h^2 I_h^{2-\alpha}u(x_i) \\
                          & =\frac{2}{h_{i} + h_{i+1}}
    \left( \frac{1}{h_{i}} I_h^{2-\alpha}u(x_{i-1}) - \left( \frac{1}{h_{i}} + \frac{1}{h_{i+1}} \right) I_h^{2-\alpha}u(x_{i}) + \frac{1}{h_{i+1}} I_h^{2-\alpha}u(x_{i+1}) \right)
  \end{aligned}
\end{equation}
Finally, we approximate the equation \eqref{eq:equation} with
\begin{equation} \label{def:discrete_equation}
  -\kappa_\alpha D_h^{\alpha} \Pi_hu(x_i) = f(x_i), \quad 1\le i \le 2N-1
\end{equation}
...
\begin{equation}
  \begin{aligned} \label{eq:Ih}
    I_h^{2-\alpha}u(x_i) & = \frac{1}{\Gamma(2-\alpha)} \int_{\Omega} |x_i-y|^{1-\alpha} \Pi_hu(y) dy                                                                     \\
...
\begin{equation}
  -\kappa_\alpha  D_h^{\alpha} \Pi_hu(x_i) = \sum_{j=1}^{2N-1} a_{ij} \; u(x_j)
\end{equation}
where
...
\begin{equation} \label{eq:truncerrordepart}
  \begin{aligned}
    - \kappa_\alpha D_h^{\alpha} \Pi_hu(x_i) - f(x_i)
     & = -\kappa_\alpha (D_h^2 I_h^{2-\alpha}u(x_i) - \frac{d^2}{dx^2} I^{2-\alpha}(x_i))                                      \\
     & = -\kappa_\alpha D_h^2 (I_h^{2-\alpha}u-I^{2-\alpha})(x_i) - \kappa_\alpha (D_h^2 - \frac{d^2}{dx^2}) I^{2-\alpha}(x_i) \\
  \end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}
    D_h^2(I^{2-\alpha}-I_h^{2-\alpha}u)(x_i)
...
\begin{equation} \label{eq:Ri}
  \begin{aligned}
    R_i & := D_h^2(I^{2-\alpha}- I_h^{2-\alpha}u)(x_i)                                                                                                                                       \\
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{aligned}
      S_{ij} & = \int_{x_{j-1}}^{x_{j}}(u(y) - \Pi_hu(y)) D_h^2 \left(\frac{|y-\cdot|^{1-\alpha}}{\Gamma(2-\alpha)}\right)(x_i) dy \\
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{aligned}
      u(y_j^\theta) - \Pi_hu(y_j^\theta) & = u(y_j^\theta) - \theta u(x_{j-1}) - (1-\theta)u(x_{j})                          \\
...
For  convenience, we use the notation and \( \simeq \)  . 
That \(x_1\simeq y_1\),
means that \(c_1 x_1 \le y_1 \le C_1 x_1\) 
for some constants \(c_1\) and \(C_1\) that are independent of mesh parameters.
...
\textcolor{blue}{
  \begin{lemma} \label{lmm:dhj-i3le-Ch2xi1-2r}
    ...
    For \(3\le i \le N-1, k=\lceil\frac{i}{2}\rceil, k \le j \le \min\{2i-1, N-1\}\), \\
  ...
  \begin{proof}
    ...
    \begin{equation}
      \begin{aligned}
        ...
        & = l \; h_{j-i}^{l-1}(\xi) \xi^{1/r-1} (y_{j-i}^{1-1/r}(\xi)- y_{j-i-1}^{1-1/r}(\xi))\\
      \end{aligned}
    \end{equation}
    ...
    \begin{equation*}
    \begin{aligned}
      h_{j-i}(\xi) & \le h_{j-i}(x_{i+1}) = h_{j+1}   \simeq h_j  \\
                   &\simeq h x_{j}^{1-1/r} \simeq  hx_{i}^{1-1/r} \\
    \end{aligned}
  \end{equation*}
  \textcolor{purple}{
  \begin{equation*}
  ...
  We get
  \begin{equation}
    (h_{j-i}^l(\xi))' \le l(r-1) C \; h_{j}^{l-1}  h  x_i^{-1/r}  \\
  \end{equation}
  \textcolor{pink}{
...
\begin{proof}[proof of \cref{lmm:estimatesofu2yj-itheta}] \label{prf:estimatesofu2yj-itheta}
  ...
  We have
  \begin{equation}
    u''(y_{j-i}^{\theta}(\xi)) \le C ({y_{j-i}^{\theta}}(\xi))^{\alpha/2-2} \le C x_{j}^{\alpha/2-2}  \le  C  x_i^{\alpha/2-2}
  \end{equation}
  \begin{equation}
    \begin{aligned}
