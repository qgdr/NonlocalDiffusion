...
\textcolor{orange}{
\begin{theorem} \label{thm:Ri-ilessNdiv2}
  For \(1\le i < N/2\), there exists a constant $C=C(T, \alpha, r, f)$ such that
  \begin{equation}
...
\textcolor{orange}{
\begin{theorem} \label{thm:Ri-igreaterNdiv2-2}
...
\subsection{Proof of \cref{thm:Ri-ilessNdiv2}}

\textcolor{orange}{
\begin{lemma} \label{lmm:sumSi-ilessNdiv2-2ip1-N}
  There exists a constant \(C=C(T, \alpha, r, f)\) such that for  \(1 \le i < N/2\),
  \begin{equation}
    \begin{aligned}
      \sum_{j=\max\{2i+1, i+3\}}^{2N} S_{ij} \le C h^2 x_i^{-\alpha/2-2/r}
    \end{aligned}
  \end{equation}
\end{lemma}
\begin{proof}
  ...
  \begin{equation}
    ...
    \end{aligned}
  \end{equation}
  Otherwise, for \(N+1\le j\le 2N-1\), by equation \cref{lmm:Dyjleh22T-y} and \cref{lmm:Dh2ymx1malecy1ma}
  \begin{equation}
    \begin{aligned}
      \sum_{j=N+1}^{2N-1} S_{ij} &\le C h^2 \int_{x_{N}}^{x_{2N-1}} (2T-y)^{\alpha/2-2/r} y^{-1-\alpha} dy \\
      &\le CT^{-1-\alpha}\frac{1}{|\alpha/2-2/r+1|} h^2 
    \end{aligned}
  \end{equation}
\end{proof}
}
...
\begin{lemma} \label{lmm:R1R2}
  By \cref{lmm:sumSij13} and \cref{lmm:sumSi-ilessNdiv2-2ip1-N} we get
  \begin{equation}
    \begin{aligned}
...
\begin{lemma} \label{lmm:Ri-I1}
  There exists a constant \(C=C(T, \alpha, r, f)\) such that for \(3\le i < N/2, k=\lceil\frac{i}{2}\rceil\)
  \begin{equation}
    I_1 \le C (h^{r\alpha/2+r} x_i^{-1-\alpha} + h^2 x_i^{-\alpha/2-2/r})
  \end{equation}
\end{lemma}
}
\begin{proof}
  For  \(2 \le j \le k-1\), by \cref{lmm:Dyjleh2yadiv2m2divr} and \cref{lmm:Dh2xmy1malecx1ma}
  \begin{equation}
    \begin{aligned}
  ...
  \begin{equation}
    I_1 \le \begin{cases}
      C h^2 x_i^{-1-\alpha} +  \frac{C}{\alpha/2-2/r+1} h^2 x_i^{-\alpha/2-2/r} , & \alpha/2-2/r+1 > 0 \\
      C h^2 x_i^{-1-\alpha} (1 + r \ln(\frac{i}{2})), & \alpha/2-2/r+1 = 0    \\
...
\begin{theorem} \label{Ri-idiv22i-I3}
  There exists a constant \(C=C(T, \alpha, r, f)\) such that for \(2\le i < N/2, k=\lceil\frac{i}{2}\rceil\),
  \begin{equation}
...
\begin{lemma} \label{lmm:Vijexpand}
...
\begin{equation} \label{def:xi2yj}
  {y_{j-i}}(x) = (x^{1/r}+ z_{j-i})^r, \quad z_{j-i} = T^{1/r}\frac{j-i}{N}
\end{equation}
\begin{equation} \label{def:xi2yjt}
...
\begin{lemma} \label{lmm:Tij-express-as-int-of-function}
  For \(2\le i \le N, 2 \le j \le \min\{2i-1, N\}\),
  \begin{equation}
    \begin{aligned}
...
\begin{lemma}
  For \(2\le i < N/2, k=\lceil\frac{i}{2}\rceil, k+1 \le j \le \min\{2i-1, N\}\),
\end{lemma}

\textcolor{orange}{
\begin{lemma} \label{lmm:Vijleh2xma22r}
  There exists a constant \(C=C(T, \alpha, r, f)\) such that for \(2\le i < N, k=\lceil\frac{i}{2}\rceil, k+1 \le j \le \min\{2i-1, N\}\),
  \begin{equation}
    V_{ij} \le \textcolor{red}{Sorry}
  \end{equation}
\end{lemma} 
\begin{proof}
  \begin{equation}
    P_{ij}^\theta = h_j^3 \frac{|y_j^\theta-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} u''(y_j^\theta)
  \end{equation}
  \begin{equation}
    Q_{ij}^\theta = h_j^4 \frac{|y_j^\theta-x_i|^{1-\alpha}}{\Gamma(2-\alpha)} 
  \end{equation}
  \begin{equation}
    \begin{aligned}
      V_{ij} &= \frac{2}{h_{i} + h_{i+1}}  \left( \frac{1}{h_{i+1}} T_{i+1, j+1} - \left(\frac{1}{h_{i}}+\frac{1}{h_{i+1}}\right)  T_{i,j} + \frac{1}{h_{i}} T_{i-1, j-1} \right) \\
      &= \int_{0}^{1} -\frac{\theta (1-\theta)}{2} \left( \frac{1}{h_{i+1}} P_{i+1, j+1}^\theta  - \left(\frac{1}{h_{i}}+\frac{1}{h_{i+1}}\right) P_{i,j}^\theta + \frac{1}{h_{i}} P_{i-1, j-1}^\theta \right) d\theta \\
      &\quad + \int_{0}^1 \frac{\theta (1-\theta)}{3!} \left( \frac{1}{h_{i+1}} Q_{i+1, j+1}^\theta  - \left(\frac{1}{h_{i}}+\frac{1}{h_{i+1}}\right) Q_{i,j}^\theta + \frac{1}{h_{i}} Q_{i-1, j-1}^\theta \right) d\theta \\
    \end{aligned}
  \end{equation}
\end{proof}
}
...
\begin{lemma} \label{lmm:Dyjleh2yadiv2m2divr}
  There is a constant \(C=C(T, \alpha, r, f)\) for \(2\le j \le N\), if \(y\in [x_{j-1}, x_{j}]\), 
  \begin{equation}
...
\begin{lemma} \label{lmm:sumSij13}
  There exists a constant \(C=C(T, \alpha, r, f)\) such that
  \begin{equation}
